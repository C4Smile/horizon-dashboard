...
// providers
import { NotificationSeverity, useNotification } from "providers";
...
const Notification = memo(() => {
  ...
  const localState = useMemo(() => {
    switch (notification) {
      ...
      case "409":
      case "429":
        return NotificationSeverity.bad;
      case "500":
        return NotificationSeverity.ugly;
      case "resetPassword":
      case "deleted":
      case "restored":
      case "200":
      case "201":
      case "204":
        return NotificationSeverity.good;
      default:
  ...
  const notificationClass = useMemo(() => {
    const lNotificationClasses: { [key: string]: string } = {
      good: "bg-green-500 text-black",
      bad: "bg-red-500",
      ugly: "bg-red-500",
    };
    if (state.length) return lNotificationClasses[state];
    return localState === NotificationSeverity.default
      ? lNotificationClasses.bad
      : lNotificationClasses[localState];
  }, [localState, state]);
        ...
        >
          ...
          >
            <div className={`text-white inline-flex`}>
              {localState === NotificationSeverity.default
                ? notification
                : t(`_accessibility:messages.${notification}`, params)}
            </div>
            <button
            ...
            >
              <span className="sr-only">
                {t("_accessibility:buttons.close")}
              </span>
              <svg

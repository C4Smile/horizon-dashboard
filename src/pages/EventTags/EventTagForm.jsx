...
/**
 * EventTag Form page component
 * @returns EventTag Form page component
 */
function EventForm() {
  const { id } = useParams();
  ...
  const onSubmit = async (d) => {
    ...
    try {
      let result;
      if (!d.id) result = await museumApiClient.EventTag.create(d);
      else result = await museumApiClient.EventTag.update(d);

      const { error, status } = result;
      setNotification(String(status), { model: t("_entities:entities.eventTag") });
      ...
      // eslint-disable-next-line no-console
      console.error(e);
      setNotification(String(e.status), { model: t("_entities:entities.eventTag") });
    }
    setSaving(false);
  };
  ...
  const eventQuery = useQuery({
    ...
    queryFn: () => museumApiClient.EventTag.getById(id),
        ...
        <Controller
          ...
          render={({ field }) => (
            <TextInput
              ...
              placeholder={t("_entities:eventTag.name.placeholder")}
              label={t("_entities:eventTag.name.label")}
              required
            />
          )}
        />

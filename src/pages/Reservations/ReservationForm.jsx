...
import { ReservationStatus } from "../../models/reservation/Reservation";
...
function CustomerForm() {
  ...
  useEffect(() => {
    if (reservationQuery.data) reset({ ...reservationQuery.data });
  ...
  }, [reservationQuery.data, id, reset]);
  ...
  return (
    <div className="px-5 pt-10 flex items-start justify-start">
      <form onSubmit={handleSubmit(onSubmit)} className="w-full">
        ...
        <Controller
          control={control}
          disabled={reservationQuery.isLoading || saving}
          name="name"
          render={({ field }) => (
            <TextInput
              {...field}
        ...
        <Controller
          control={control}
          name="customerId"
          disabled={reservationQuery.isLoading || customerQuery.isLoading || saving}
          render={({ field: { onChange, value, ...rest } }) => (
            <SelectInput
              ...
              options={
                customerQuery?.data?.map((c) => ({ value: `${c.name} - ${c.iso}`, id: c.id })) || []
              }
              value={value}
              onChange={(e) => {
        ...
        >
          {(reservationQuery.isLoading || saving) && (
            <Loading

...
/**
 * Currency Form page component
 * @returns Currency Form page component
 */
function CurrencyForm() {
  ...
  const onSubmit = async (d) => {
    ...
    try {
      let result;
      if (!d.id) result = await museumApiClient.Currency.create(d);
      else result = await museumApiClient.Currency.update(d);
      ...
      if (status !== 201) console.error(error);
        ...
        queryClient.invalidateQueries({ queryKey: [ReactQueryKeys.Currencies, id] });
      else
        reset({
          id: undefined,
          name: "",
      ...
      // eslint-disable-next-line no-console
      console.error(e);
      setNotification(String(e.status), { model: t("_entities:entities.currency") });
    }
    setSaving(false);
  };

  const currencyQuery = useQuery({
    queryKey: [ReactQueryKeys.Currencies, id],
    queryFn: () => museumApiClient.Currency.getById(id),
    enabled: id !== undefined,
    retry: false,
  });

  useEffect(() => {
    const { error } = currencyQuery;
    // eslint-disable-next-line no-console
    if (error && error !== null) console.error(error);
  }, [currencyQuery]);

  useEffect(() => {
    if (currencyQuery.data) reset({ ...currencyQuery.data });
    ...
    <div className="px-5 pt-10 flex items-start justify-start">
      <form onSubmit={handleSubmit(onSubmit)} className="w-full">
        <h1 className="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold mb-5">
          {id ? `${t("_pages:currencies.editForm")} ${id}` : t("_pages:currencies.newForm")}
        </h1>
        <Controller
          control={control}
          disabled={currencyQuery.isLoading || saving}
          name="name"
          render={({ field }) => (
            <TextInput
              {...field}
              ...
              label={t("_entities:currency.name.label")}
              required
            />
          )}
        />
        <Controller
          ...
          render={({ field }) => (
            <TextInput
              ...
              id="reduction"
              className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white focus:outline-none focus:ring-0 focus:border-blue-600 peer"
              placeholder={t("_entities:currency.reduction.placeholder")}
              label={t("_entities:currency.reduction.label")}
              required
            />
          )}
        />
        ...
        >
          {(currencyQuery.isLoading || saving) && (
            <Loading
